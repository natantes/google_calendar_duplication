...
const START_TIME_SELECTOR = "[aria-label='Start time']";
const END_TIME_SELECTOR = "[aria-label='End time']";
const DATE_SELECTOR = ".e0yuIb";
...
function formatTime(timeStr) {
  let hour = Number(timeStr.slice(1, 3));
...
function addFifteenMinutes(time) {
  if (time) {
    const timePart = time.substring(1, 7); // get the part after 'T' and before 'Z'
    const hourPart = parseInt(timePart.substring(0, 2));
    const minutePart = parseInt(timePart.substring(2, 4));
    let newMinute = minutePart + 15;
    ...
    if (newHour >= 24) { // check if it is next day
      newHour -= 24;
    }
    ...
    const newTime = 'T' + newTimePart;

    return newTime;
  } else {
    console.log('Invalid time passed to addFifteenMinutes. Make sure "data-ical" attribute exists and is correctly formatted.');
    return time;
  }
}
...
function convertTimeFormat(inputTime) {
  ...
  var hour = parseInt(inputTime.substring(0, inputTime.indexOf(':')));
  var minutes = inputTime.substring(inputTime.indexOf(':') + 1, inputTime.length - 2);
  var period = inputTime.substring(inputTime.length - 2);
...
function saveEvent() {
  ...
  intervalSaveEvent = setInterval(function () {
    ...
    let startTimeElement = document.querySelector(START_TIME_SELECTOR);
    if (startTimeElement) { 
      const startValue = startTimeElement.value;
      startTimeElement.click();
      newStartTime = addFifteenMinutes(convertTimeFormat(startValue));
      let startTimeButton = document.querySelector(`div[data-ical="${newStartTime}"]`);

      if (startTimeButton) {
        startTimeButton.click();
      } else {
        console.log('No button for the desired start time.');
      }

      let endTimeElement = document.querySelector(END_TIME_SELECTOR);
      endTimeElement.click();
  
      newEndTime = addFifteenMinutes(newStartTime);
      let endTimeButton = document.querySelectorAll(`div[data-ical="${newEndTime}"]`)
      endTimeButton = endTimeButton[1]
  
      if (endTimeButton) {
        endTimeButton.click();
      } else {
        console.log('No button for the desired end time.');
      }

    } else {
      console.log("no start time element")
    }
